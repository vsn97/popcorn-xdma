...
void update_pkey(unsigned long pkey, unsigned long addr)
{
	...
	unsigned long *res;
	int ret;
	//PCNPRINTK("Inside the update pkey func: %lx and %lx\n", pkey, addr);
	res = radix_tree_lookup(&pkey_rd_tree, addr);
	if(res != NULL) {
		if(*res == pkey) {
			//PCNPRINTK("No need to update: %lx and %lx\n", *res, pkey);
		} else {
		//ret = radix_tree_delete(&pkey_rd_tree, addr);
		//p_key = kmalloc(sizeof(unsigned long), GFP_ATOMIC);
		*res = pkey;
		//radix_tree_insert(&pkey_rd_tree, addr, p_key);
		//PCNPRINTK("PKEY UPDATED: %lx and %lx\n", *res, pkey);
		}
	} else {
		//PCNPRINTK("PKEY Doesn't exist, inserting it\n");
		p_key = kmalloc(sizeof(unsigned long), GFP_ATOMIC);
		*p_key = pkey;
		radix_tree_insert(&pkey_rd_tree, addr, p_key);
	}
...
/* XDMA Invalidate Page Function */

static void __xdma_invalidate_page(struct task_struct *tsk, unsigned long addr, pid_t opid, pid_t rpid, int from_nid)
{
	struct mm_struct *mm = get_task_mm(tsk);
	struct vm_area_struct *vma;
	pmd_t *pmd;
...
static int __request_remote_page(struct task_struct *tsk, int from_nid, pid_t from_pid, unsigned long addr, unsigned long fault_flags, int ws_id, struct pcn_kmsg_rdma_handle **rh, struct pcn_kmsg_xdma_handle **xh)
{
	...
	*xh = NULL;
	...
	{
		struct pcn_kmsg_xdma_handle *handle = 
				pcn_kmsg_pin_xdma_buffer(NULL, PAGE_SIZE);
		if(IS_ERR(handle)) {
			pcn_kmsg_put(req);
			return PTR_ERR(handle);
		}
...
static remote_page_response_t *__fetch_page_from_origin(struct task_struct *tsk, struct vm_fault *vmf, struct vm_area_struct *vma, unsigned long addr, unsigned long fault_flags, struct page *page)
{
	...
	struct pcn_kmsg_xdma_handle *xh = pcn_kmsg_pin_xdma_buffer(NULL, PAGE_SIZE);
	unsigned long pkey = 0;
	unsigned long *pkey_res;

	if(TRANSFER_PAGE_WITH_XDMA) {
		//PCNPRINTK("Sending the Fetch page from origin: %llx and %llx\n", xh->dma_addr, (unsigned long)vmf);
		/*
		if(fault_for_write(fault_flags)) {
			PCNPRINTK("WRITE FAULT: %d and %x\n", fault_flags, fault_flags);
		} else if(fault_for_read(fault_flags)) {
			PCNPRINTK("READ FAULT: %d and %x\n", fault_flags, fault_flags);
		} else {
	...
	//end_time = ktime_get_ns();
	//PCNPRINTK("Time taken to receive a page: %d\n", end_time - start_time);
	if (rp->result == 0) {
		//pending();
		void *paddr = kmap(page);
		//PCNPRINTK("Copying the page: %x\n", rp->result);
		if (TRANSFER_PAGE_WITH_RDMA) {
...
static int __xdma_claim_remote_page(struct task_struct *tsk, struct mm_struct *mm, struct vm_area_struct *vma, unsigned long addr, unsigned long fault_flags, struct page *page, unsigned long pkey)
{
	...
	PCNPRINTK("Inside the __xdma_claim_remote_page func\n");

	ws = get_wait_station(tsk);
	if (r_nid != my_nid) {
		rpid = rc->remote_tgids[r_nid];
		PCNPRINTK("Requesting operation from this pid : %d and %d\n", r_nid, rpid);
	} else {
		rpid = 0;
...
static int __handle_remotefault_at_remote(struct task_struct *tsk, struct mm_struct *mm, struct vm_area_struct *vma, remote_page_request_t *req, remote_page_response_t *res)
{
	...
	if (TRANSFER_PAGE_WITH_RDMA) {
		...
		paddr = kmap(page);
		pcn_kmsg_xdma_write(PCN_KMSG_FROM_NID(req),
...
out:
	...
	else if (res->result != 0 || TRANSFER_PAGE_WITH_XDMA) {
		res_type = PCN_KMSG_TYPE_REMOTE_PAGE_RESPONSE_SHORT;
		res_size = sizeof(remote_page_response_short_t);
	}else {
...
static int __xdma_handle_rmfault_at_origin(struct task_struct *tsk, struct mm_struct *mm, struct vm_area_struct *vma, unsigned long addr, unsigned long iaddr, unsigned long fault_flags, unsigned long pkey, pid_t rpid, pid_t opid, int ws_id, int from_nid, int x, remote_page_response_t *res)
{
	unsigned char *paddr;
	struct page *page;
	...
	bool leader;
	bool grant = false;
	//PCNPRINTK("Inside the __xdma_handle_rmfault_at_origin func: %d\n", x);
again:
	pte = __get_pte_at_alloc(mm, vma, addr, &pmd, &ptl);
	if (!pte) {
		PGPRINTK("  [%d] No PTE!!\n", tsk->pid);
	...
	if (leader) {
		...
		entry = ptep_clear_flush(vma, addr, pte);
		SetPageDistributed(mm, addr);
		//PCNPRINTK("Im the leader\n");
		/* Prepare the page if it is not mine. This should be leader */

		if (x == VMF_CONTINUE) {
			/* Node already owns up-to date page */
			BUG_ON(fault_for_read(fault_flags) && "Read fault from owner??");
			//PCNPRINTK("Node already owns up-to date page: %d\n", x);
			grant = true;
			entry = pte_make_invalid(entry);
...
static int __xdma_handle_rmfault_at_remote(struct task_struct *tsk, struct mm_struct *mm, struct vm_area_struct *vma, unsigned long vaddr, unsigned long iaddr, unsigned long fflags, unsigned long pkey, pid_t rpid, pid_t opid, int ws_id, int from_nid, int x, remote_page_response_t *res)
{
	...
	//PCNPRINTK("Inside the __xdma_handle_rmfault_at_remote func: %d\n", x);
	pte = __get_pte_at(mm, addr, &pmd, &ptl);
	if (!pte) {
		PGPRINTK("  [%d] No PTE!!\n", tsk->pid);
		return VM_FAULT_OOM;
	...
	if (x == RPR_WR) {
		//Write Fault at origin
		//PCNPRINTK("Write Fault @ origin\n");
		entry = pte_make_invalid(entry);
	} else if(x == RPR_RD){
		//Read Fault at origin
		//PCNPRINTK("Read Fault @ origin\n");		
		entry = pte_wrprotect(entry);
	} else {
...
void xdma_process_remote_page_request(dsm_proc_request_t *req)
{	
	...
	//dsm_proc_request_t *req = proc_req;
	...
	//PCNPRINTK("Inside the page_server prot_proc rpr handler: %lx and %lx and %lx and %lx and %lx and %d and %d and %d and %d\n", req->addr, req->instr_addr, req->page_mode, req->fault_flags, req->page_key, req->remote_pid, req->origin_pid, req->ws_id, from_nid);
	/*
	if(fault_for_write(fault_flags)) {
			PCNPRINTK("WRITE FAULT: %d and %x\n", fault_flags, fault_flags);
	} else if(fault_for_read(fault_flags)) {
...
again:
	...
	if (tsk->at_remote) {
		...
		res->result = __xdma_handle_rmfault_at_remote(tsk, mm, vma, req->addr, req->instr_addr, req->fault_flags, req->page_key, req->remote_pid, req->origin_pid, req->ws_id, from_nid, req->page_mode, res);
	} else {
		//PCNPRINTK("Calling the xdma_rmfault_at_origin: %d", tsk->at_remote);
		res->result = __xdma_handle_rmfault_at_origin(tsk, mm, vma, req->addr, req->instr_addr, req->fault_flags, req->page_key, req->remote_pid, req->origin_pid, req->ws_id, from_nid, req->page_mode, res);
	}
...
void xdma_process_invalidate_request(dsm_proc_request_t *req)
{
	//struct pcn_kmsg_work *pcn_work = (struct pcn_kmsg_work *)work;
	...
	__xdma_invalidate_page(tsk, req->addr, req->origin_pid, req->remote_pid, from_nid);
...
//EXPORT_SYMBOL(xdma_process_invalidate_request);
...
static void process_prot_proc_request(struct work_struct *work)
{
	...
	if (req->page_mode == INVALIDATE) {
		xdma_process_invalidate_request(req);
	} else {
		xdma_process_remote_page_request(req);
	}
...
{
	unsigned long *p_key = kmalloc(sizeof(unsigned long), GFP_ATOMIC);
	*p_key = pkey;
	//PCNPRINTK("Inside the insert pkey func: %lx and %lx and %lx\n", pkey, *p_key, addr);
	radix_tree_insert(&pkey_rd_tree, addr, p_key);
}

EXPORT_SYMBOL(insert_pkey);
*/
...
void delete_pkeys()
{
	...
	//PCNPRINTK("Inside delete_pkeys(): %d\n",ret);
  	for(i = 0; i < ret; i++) {
  		res = pkeys[i];
	...
	if (TRANSFER_PAGE_WITH_XDMA) {
		...
		if (pkey_res != NULL) {
			...
			if (leader && (lastX != 0xAAAA0)) {
				...
				//PCNPRINTK("xdma: Claiming the remote page\n");
				__xdma_claim_remote_page(current, mm, vma, addr, fault_flags, page, pkey);
...
static int __handle_localfault_at_remote(struct vm_fault *vmf)
{
	...
	//PCNPRINTK("Got the result from the origin: %x\n", rp->result);
	...
	if (rp->result == VM_FAULT_CONTINUE) {
		...
		//PCNPRINTK("Page is granted to the node: %x\n", rp->result);
		spin_lock(ptl);
		if (populated) {
...
/* XDMA Localfault at Remote Handler */

static int __xdma_handle_lcfault_at_remote(struct vm_fault *vmf)
{
	spinlock_t *ptl;
	struct page *page;
	bool populated = false;
...
/* XDMA Localfault at Origin Handler */

static int __xdma_handle_lcfault_at_origin(struct vm_fault *vmf)
{
	spinlock_t *ptl;
	int peers, ret;
	unsigned long addr = vmf->address & PAGE_MASK;
	...
	unsigned mask, lastX;
  	...
  	mask = (1 << 20) - 1;
	//PCNPRINTK("Inside the __xdma_handle_lcfault_at_origin\n");
	...
	if(pkey_res != NULL) {
		...
		pkey = 0;
		//PCNPRINTK("Didn't find the PKEY in Localfault @ ORIGIN: %lx\n", addr);
		BUG_ON(!pkey);
	}
...
int page_server_handle_pte_fault(struct vm_fault *vmf)
{
	...
	if (!current->at_remote) {
		...
		if (TRANSFER_PAGE_WITH_XDMA) {
			//PCNPRINTK("Calling the __xdma_handle_lcfault_at_origin()\n");
			ret = __xdma_handle_lcfault_at_origin(vmf);
		} else {
	...
	if (!pte_is_present(vmf->orig_pte)) {
		/* Remote page fault */
		//PCNPRINTK("Fault @ REMOTE\n");
		if (TRANSFER_PAGE_WITH_XDMA) {
			ret = __xdma_handle_lcfault_at_remote(vmf);
		} else {
...
//DEFINE_KMSG_WQ_HANDLER(xdma_remote_page_request);
DEFINE_KMSG_WQ_HANDLER(page_invalidate_request);
//DEFINE_KMSG_WQ_HANDLER(xdma_invalidate_request);
...
int __init page_server_init(void)
{
	...
	//REGISTER_KMSG_WQ_HANDLER(
			//PCN_KMSG_TYPE_XDMA_REMOTE_PAGE_REQUEST, xdma_remote_page_request);
	...
	//REGISTER_KMSG_WQ_HANDLER(
			//PCN_KMSG_TYPE_XDMA_INVALIDATE_REQUEST, xdma_invalidate_request);
	REGISTER_KMSG_WQ_HANDLER(
