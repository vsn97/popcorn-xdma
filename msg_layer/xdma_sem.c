...
{
	struct pcn_kmsg_message *msg = (struct pcn_kmsg_message *)(work->addr + sizeof(struct rb_alloc_header));
	...
	send_queue->size++;
	PCNPRINTK("Send Tail and Size and message enqueued: %d and %d and %d and %d and %lx\n",send_queue->tail,send_queue->size, msg->header.from_nid, msg->header.type, msg->header.size);
...
int xdma_kmsg_write(int to_nid, dma_addr_t raddr, void *addr, size_t size)
{
	...
	curr_xw = xw;
	spin_lock(&xdma_lock);
	ret = __xdma_transfer(xw->dma_addr, size, PAGE);
	spin_unlock(&xdma_lock);
	if(ret) {
...
static void __channel_interrupts_disable(int z, int x)
{
	...
	if(z) {
		if(!x) {
			//PCNPRINTK("H2C Perf Stats: %x and %lx and %lx and %lx\n", read_register((u32 *)(xdma_ctl + 0xC0)), read_register((u32 *)xdma_bypass + 1), read_register((u32 *)xdma_bypass + 2), read_register((u32 *)xdma_bypass + 3));
		...
		if(!x) {
			//PCNPRINTK("C2H Perf Stats: %x and %lx and %lx and %lx\n", read_register((u32 *)(xdma_ctl + 0x10C0)), read_register((u32 *)xdma_bypass + 10), read_register((u32 *)xdma_bypass + 11), read_register((u32 *)xdma_bypass + 12));
...
static irqreturn_t xdma_isr(int irq, void *dev_id)
{
	...
	if(read_irq & 0x01) {
		...
		PCNPRINTK("Failure in IRQ Disable: %d\n", read_irq);
	}
