...
void prot_proc_handle_localfault(unsigned long vmf, unsigned long vaddr, dma_addr_t dma_addr, unsigned long iaddr, pid_t opid, pid_t rpid, int nid, unsigned long fflags, int ws_id, int tsk_remote)
{	    
		PCNPRINTK("Inside the prot_proc handle localfault func\n");
		...
		write_register(nid, (u32 *)(xdma_axi + proc_nid));
 		PCNPRINTK("Wrote everything except control\n");
 		if(tsk_remote) {
 		...
 		PCNPRINTK("Done writing the info\n");
